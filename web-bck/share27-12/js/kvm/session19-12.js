...
export function Session() {
	...
	var __hid2 = new Hid2(__streamer2.getResolution, __recorder);
	var __hid3 = new Hid3(__streamer3.getResolution, __recorder);
		...
		let http = tools.makeRequest("GET", "/api/auth/check", function() {
			if (http.readyState === 4) {
				if (http.status === 200) {
					...
					wm2.error("Unexpected logout occured, please login again").then(function() {
						document.location.href = "/login";
					});
				} else {
	...
	var __wsMessageHandler = function(event) {
		...
		switch (data.event_type) {
			...
			case "hid_keymaps_state": __hid.setKeymaps(data.event);__hid2.setKeymaps(data.event);__hid3.setKeymaps(data.event); break;
			case "hid_state": __hid.setState(data.event); __hid2.setState(data.event); __hid3.setState(data.event);break;
			case "atx_state": __atx.setState(data.event); break;
			case "msd_state": __msd.setState(data.event); break;
			case "streamer_state": __streamer.setState(data.event); __streamer2.setState(data.event); __streamer3.setState(data.event);break;
		}
	};
