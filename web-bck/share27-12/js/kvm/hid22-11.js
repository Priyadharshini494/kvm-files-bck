...
export function Hid3(__getResolution, __recorder) {
	...
	var __init__ = function() {
		...
		tools.storage.bindSimpleSwitch($("hid-pak-ask-switch3"), "hid.pak.ask", true);

		$("hid-pak-keymap-selector3").addEventListener("change", function() {
			tools.storage.set("hid.pak.keymap", $("hid-pak-keymap-selector3").value);
		});

		tools.el.setOnClick($("hid-pak-button3"), __clickPasteAsKeysButton);
		tools.el.setOnClick($("hid-connect-switch3"), __clickConnectSwitch);
		tools.el.setOnClick($("hid-reset-button3"), __clickResetButton);
	...
	self.setSocket = function(ws) {
		tools.el.setEnabled($("hid-pak-text3"), ws);
		tools.el.setEnabled($("hid-pak-button3"), ws);
		tools.el.setEnabled($("hid-reset-button3"), ws);
		if (!ws) {
			self.setState(null);
		}
		__keyboard.setSocket(ws);
	...
	self.setState = function(state) {
		...
		if (state && state.online) {
			...
			let has_relative = false;
			if (mouse_outputs.length) {
				if ($("hid-outputs-mouse-box3").outputs !== mouse_outputs) {
					let html = "";
					for (let args of [
						["USB", "usb", false],
						["USB Win98", "usb_win98", false],
					...
					$("hid-outputs-mouse-box3").innerHTML = html;
					$("hid-outputs-mouse-box3").outputs = mouse_outputs;
					tools.radio.setOnClick("hid-outputs-mouse-radio", () => __clickOutputsRadio("mouse"));
				}
				tools.radio.setValue("hid-outputs-mouse-radio", state.mouse.outputs.active);
				has_relative_squash = ["usb_rel", "ps2"].includes(state.mouse.outputs.active);
			...
			tools.feature.setEnabled($("hid-outputs3"), (keyboard_outputs.length || mouse_outputs.length));
			tools.feature.setEnabled($("hid-outputs-keyboard"), keyboard_outputs.length);
			tools.feature.setEnabled($("hid-outputs-mouse3"), mouse_outputs.length);
			tools.feature.setEnabled($("hid-mouse-squash3"), has_relative);
			tools.feature.setEnabled($("hid-mouse-sens3"), has_relative);
			tools.feature.setEnabled($("hid-connect3"), (state.connected !== null));
			$("hid-connect-switch3").checked = !!state.connected;
		}

		tools.radio.setEnabled("hid-outputs-keyboard-radio", (state && state.online && !state.busy));
		tools.radio.setEnabled("hid-outputs-mouse-radio", (state && state.online && !state.busy));
		tools.el.setEnabled($("hid-mouse-squash-switch3"), (has_relative_squash && !state.busy));
		tools.el.setEnabled($("hid-mouse-sens-slider3"), (has_relative_squash && !state.busy));
		tools.el.setEnabled($("hid-connect-switch3"), (state && state.online && !state.busy));
	...
	self.setKeymaps = function(state) {
		...
		$("hid-pak-keymap-selector3").innerHTML = html;
	};
	...
	var __clickPasteAsKeysButton = function() {
		let text = $("hid-pak-text3").value;
		if (text) {
			let paste_as_keys = function() {
				tools.el.setEnabled($("hid-pak-text3"), false);
				tools.el.setEnabled($("hid-pak-button3"), false);
				tools.el.setEnabled($("hid-pak-keymap-selector3"), false);

				let keymap = $("hid-pak-keymap-selector3").value;

				tools.debug(`HID: paste-as-keys ${keymap}: ${text}`);

				let http = tools.makeRequest("POST", `/api/hid/print?limit=0&keymap=${keymap}`, function() {
					if (http.readyState === 4) {
						tools.el.setEnabled($("hid-pak-text3"), true);
						tools.el.setEnabled($("hid-pak-button3"), true);
						tools.el.setEnabled($("hid-pak-keymap-selector3"), true);
						$("hid-pak-text3").value = "";
						if (http.status === 413) {
							wm.error("Too many text for paste!");
						} else if (http.status !== 200) {
			...
			if ($("hid-pak-ask-switch3").checked) {
				let confirm_msg = `You're going to paste ${text.length} character${text.length ? "s" : ""}.<br>`;
				confirm_msg += "Are you sure you want to continue?";
				wm.confirm(confirm_msg).then(function(ok) {
					if (ok) {
	...
	var __clickConnectSwitch = function() {
		let connected = $("hid-connect-switch3").checked;
		let http = tools.makeRequest("POST", `/api/hid/set_connected?connected=${connected}`, function() {
			if (http.readyState === 4) {
				if (http.status !== 200) {
					wm.error(`Can't ${connected ? "connect" : "disconnect"} HID:<br>`, http.responseText);
