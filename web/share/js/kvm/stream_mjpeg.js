...
export function MjpegStreamer5(__setActive, __setInactive, __setInfo) {
	var self = this;
	...
	self.getName = () => "ANDROID";
	self.getMode = () => "mjpeg";

	self.getResolution = function() {
		let el = $("stream-image5");
		return {
			"real_width": el.naturalWidth,
			"real_height": el.naturalHeight,
			"view_width": el.offsetWidth,
	...
	self.stopStream = function() {
		self.ensureStream(null);
		let blank = "/share/png/blank-stream.png";
		if (!String.prototype.endsWith.call($("stream-image5").src, blank)) {
			$("stream-image5").src = blank;
		}
	};
...
//			__setStreamInactive();
			...
			__logInfo("Refreshing ...");
			$("stream-image5").src = path;
		}
	};

	var __logInfo = (...args) => tools.info("Stream [MJPEG5]:", ...args);
}
