...
export function Hid2(__getResolution, __recorder) {
	var self = this;
	...
	var __init__ = function() {
		...
		__mouse = new Mouse2(__getResolution, __recorder.recordWsEvent);
		...
		tools.storage.bindSimpleSwitch($("hid-pak-ask-switch2"), "hid.pak.ask", true);

		$("hid-pak-keymap-selector2").addEventListener("change", function() {
			tools.storage.set("hid.pak.keymap", $("hid-pak-keymap-selector2").value);
		});

		tools.el.setOnClick($("hid-pak-button2"), __clickPasteAsKeysButton);
		tools.el.setOnClick($("hid-connect-switch2"), __clickConnectSwitch);
		tools.el.setOnClick($("hid-reset-button2"), __clickResetButton);
	...
	self.setSocket = function(ws) {
		tools.el.setEnabled($("hid-pak-text2"), ws);
		tools.el.setEnabled($("hid-pak-button2"), ws);
		tools.el.setEnabled($("hid-reset-button2"), ws);
		if (!ws) {
			self.setState(null);
		}
		__keyboard.setSocket(ws);
	...
	self.setState = function(state) {
		...
		if (state && state.online) {
			...
			let has_relative = false;
			if (mouse_outputs.length) {
				if ($("hid-outputs-mouse-box2").outputs !== mouse_outputs) {
					let html = "";
					for (let args of [
						["USB", "usb", false],
						["USB Win98", "usb_win98", false],
					...
					$("hid-outputs-mouse-box2").innerHTML = html;
					$("hid-outputs-mouse-box2").outputs = mouse_outputs;
					tools.radio.setOnClick("hid-outputs-mouse-radio", () => __clickOutputsRadio("mouse"));
				}
				tools.radio.setValue("hid-outputs-mouse-radio", state.mouse.outputs.active);
				has_relative_squash = ["usb_rel", "ps2"].includes(state.mouse.outputs.active);
			...
			tools.feature.setEnabled($("hid-outputs2"), (keyboard_outputs.length || mouse_outputs.length));
			tools.feature.setEnabled($("hid-outputs-keyboard"), keyboard_outputs.length);
			tools.feature.setEnabled($("hid-outputs-mouse2"), mouse_outputs.length);
			tools.feature.setEnabled($("hid-mouse-squash2"), has_relative);
			tools.feature.setEnabled($("hid-mouse-sens2"), has_relative);
			tools.feature.setEnabled($("hid-connect2"), (state.connected !== null));
			$("hid-connect-switch2").checked = !!state.connected;
		}

		tools.radio.setEnabled("hid-outputs-keyboard-radio", (state && state.online && !state.busy));
		tools.radio.setEnabled("hid-outputs-mouse-radio", (state && state.online && !state.busy));
		tools.el.setEnabled($("hid-mouse-squash-switch2"), (has_relative_squash && !state.busy));
		tools.el.setEnabled($("hid-mouse-sens-slider2"), (has_relative_squash && !state.busy));
		tools.el.setEnabled($("hid-connect-switch2"), (state && state.online && !state.busy));
	...
	self.setKeymaps = function(state) {
		...
		$("hid-pak-keymap-selector2").innerHTML = html;
	};
	...
	var __clickPasteAsKeysButton = function() {
		let text = $("hid-pak-text2").value;
		if (text) {
			let paste_as_keys = function() {
				tools.el.setEnabled($("hid-pak-text2"), false);
				tools.el.setEnabled($("hid-pak-button2"), false);
				tools.el.setEnabled($("hid-pak-keymap-selector2"), false);

				let keymap = $("hid-pak-keymap-selector2").value;

				tools.debug(`HID: paste-as-keys ${keymap}: ${text}`);

				let http = tools.makeRequest("POST", `/api/hid/print?limit=0&keymap=${keymap}`, function() {
					if (http.readyState === 4) {
						tools.el.setEnabled($("hid-pak-text2"), true);
						tools.el.setEnabled($("hid-pak-button2"), true);
						tools.el.setEnabled($("hid-pak-keymap-selector2"), true);
						$("hid-pak-text2").value = "";
						if (http.status === 413) {
							wm.error("Too many text for paste!");
						} else if (http.status !== 200) {
			...
			if ($("hid-pak-ask-switch2").checked) {
				let confirm_msg = `You're going to paste ${text.length} character${text.length ? "s" : ""}.<br>`;
				confirm_msg += "Are you sure you want to continue?";
				wm.confirm(confirm_msg).then(function(ok) {
					if (ok) {
	...
	var __clickConnectSwitch = function() {
		let connected = $("hid-connect-switch2").checked;
		let http = tools.makeRequest("POST", `/api/hid/set_connected?connected=${connected}`, function() {
			if (http.readyState === 4) {
				if (http.status !== 200) {
					wm.error(`Can't ${connected ? "connect" : "disconnect"} HID:<br>`, http.responseText);
...
export function Hid3(__getResolution, __recorder) {
	var self = this;
	...
	var __init__ = function() {
		...
		__mouse = new Mouse3(__getResolution, __recorder.recordWsEvent);
	...
	var __clickPasteAsKeysButton = function() {
		...
		if (text) {
			let paste_as_keys = function() {
				...
				let http = tools.makeRequest("POST", `/api/hid/print?limit=0&keymap=${keymap}`, function() {
					if (http.readyState === 4) {
						...
						$("hid-pak-text").value = "";
						if (http.status === 413) {
							wm2.error("Too many text for paste!");
						} else if (http.status !== 200) {
							wm2.error("HID paste error:<br>", http.responseText);
						} else if (http.status === 200) {
							__recorder.recordPrintEvent(text);
						}
					}
			...
			if ($("hid-pak-ask-switch").checked) {
				let confirm_msg = `You're going to paste ${text.length} character${text.length ? "s" : ""}.<br>`;
				confirm_msg += "Are you sure you want to continue?";
				wm2.confirm(confirm_msg).then(function(ok) {
					if (ok) {
						paste_as_keys();
					} else {
						$("hid-pak-text").value = "";
	...
	var __clickOutputsRadio = function(hid) {
		...
		let http = tools.makeRequest("POST", `/api/hid/set_params?${hid}_output=${output}`, function() {
			if (http.readyState === 4) {
				if (http.status !== 200) {
					wm2.error("Can't configure HID:<br>", http.responseText);
				}
			}
		});
	};
	...
	var __clickConnectSwitch = function() {
		...
		let http = tools.makeRequest("POST", `/api/hid/set_connected?connected=${connected}`, function() {
			if (http.readyState === 4) {
				if (http.status !== 200) {
					wm2.error(`Can't ${connected ? "connect" : "disconnect"} HID:<br>`, http.responseText);
				}
			}
		});
	};
	...
	var __clickResetButton = function() {
		wm2.confirm("Are you sure you want to reset HID (keyboard & mouse)?").then(function(ok) {
			if (ok) {
				let http = tools.makeRequest("POST", "/api/hid/reset", function() {
					if (http.readyState === 4) {
						if (http.status !== 200) {
							wm2.error("HID reset error:<br>", http.responseText);
						}
					}
				});
			}
...
export function Hid4(__getResolution, __recorder) {
	var self = this;
	...
	var __init__ = function() {
		...
		__mouse = new Mouse4(__getResolution, __recorder.recordWsEvent);
		...
		$("hid-pak-keymap-selector4").addEventListener("change", function() {
			tools.storage.set("hid.pak.keymap", $("hid-pak-keymap-selector4").value);
		});

		tools.el.setOnClick($("hid-pak-button4"), __clickPasteAsKeysButton);
		tools.el.setOnClick($("hid-connect-switch4"), __clickConnectSwitch);
		tools.el.setOnClick($("hid-reset-button4"), __clickResetButton);
	...
	self.setSocket = function(ws) {
		tools.el.setEnabled($("hid-pak-text4"), ws);
		tools.el.setEnabled($("hid-pak-button4"), ws);
		tools.el.setEnabled($("hid-reset-button4"), ws);
		if (!ws) {
			self.setState(null);
		}
		__keyboard.setSocket(ws);
	...
	self.setState = function(state) {
		...
		if (state && state.online) {
			...
			let has_relative = false;
			if (mouse_outputs.length) {
				if ($("hid-outputs-mouse-box4").outputs !== mouse_outputs) {
					let html = "";
					for (let args of [
						["USB", "usb", false],
						["USB Win98", "usb_win98", false],
					...
					$("hid-outputs-mouse-box4").innerHTML = html;
					$("hid-outputs-mouse-box4").outputs = mouse_outputs;
					tools.radio.setOnClick("hid-outputs-mouse-radio", () => __clickOutputsRadio("mouse"));
				}
				tools.radio.setValue("hid-outputs-mouse-radio", state.mouse.outputs.active);
				has_relative_squash = ["usb_rel", "ps2"].includes(state.mouse.outputs.active);
			...
			tools.feature.setEnabled($("hid-outputs4"), (keyboard_outputs.length || mouse_outputs.length));
			tools.feature.setEnabled($("hid-outputs-keyboard"), keyboard_outputs.length);
			tools.feature.setEnabled($("hid-outputs-mouse4"), mouse_outputs.length);
			tools.feature.setEnabled($("hid-mouse-squash4"), has_relative);
			tools.feature.setEnabled($("hid-mouse-sens4"), has_relative);
			tools.feature.setEnabled($("hid-connect4"), (state.connected !== null));
			$("hid-connect-switch4").checked = !!state.connected;
		}

		tools.radio.setEnabled("hid-outputs-keyboard-radio", (state && state.online && !state.busy));
		tools.radio.setEnabled("hid-outputs-mouse-radio", (state && state.online && !state.busy));
		tools.el.setEnabled($("hid-mouse-squash-switch4"), (has_relative_squash && !state.busy));
		tools.el.setEnabled($("hid-mouse-sens-slider4"), (has_relative_squash && !state.busy));
		tools.el.setEnabled($("hid-connect-switch4"), (state && state.online && !state.busy));
	...
	self.setKeymaps = function(state) {
		...
		$("hid-pak-keymap-selector4").innerHTML = html;
	};
	...
	var __clickPasteAsKeysButton = function() {
		let text = $("hid-pak-text4").value;
		if (text) {
			let paste_as_keys = function() {
				tools.el.setEnabled($("hid-pak-text4"), false);
				tools.el.setEnabled($("hid-pak-button4"), false);
				tools.el.setEnabled($("hid-pak-keymap-selector4"), false);

				let keymap = $("hid-pak-keymap-selector4").value;

				tools.debug(`HID: paste-as-keys ${keymap}: ${text}`);

				let http = tools.makeRequest("POST", `/api/hid/print?limit=0&keymap=${keymap}`, function() {
					if (http.readyState === 4) {
						tools.el.setEnabled($("hid-pak-text4"), true);
						tools.el.setEnabled($("hid-pak-button4"), true);
						tools.el.setEnabled($("hid-pak-keymap-selector4"), true);
						$("hid-pak-text4").value = "";
						if (http.status === 413) {
							wm.error("Too many text for paste!");
						} else if (http.status !== 200) {
			...
			if ($("hid-pak-ask-switch4").checked) {
				let confirm_msg = `You're going to paste ${text.length} character${text.length ? "s" : ""}.<br>`;
				confirm_msg += "Are you sure you want to continue?";
				wm.confirm(confirm_msg).then(function(ok) {
					if (ok) {
	...
	var __clickConnectSwitch = function() {
		let connected = $("hid-connect-switch4").checked;
		let http = tools.makeRequest("POST", `/api/hid/set_connected?connected=${connected}`, function() {
			if (http.readyState === 4) {
				if (http.status !== 200) {
					wm.error(`Can't ${connected ? "connect" : "disconnect"} HID:<br>`, http.responseText);
