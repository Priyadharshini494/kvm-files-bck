...
class Streamer:  # pylint: disable=too-many-instance-attributes
    ...
    async def get_state(self) -> dict:
        ...
        return {
            ...
            "streamer": streamer_state2,
            "features": self.__params.get_features(),
        }
    
    async def get_state2(self) -> dict:
        streamer_state2 = None
        if self.__streamer_task:
            session = self.__ensure_http_session2()
            try:
                async with session.get(self.__make_url("state")) as response:
                    htclient.raise_not_200(response)
                ...
                get_logger().exception("Invalid streamer2 response from /state")
        snapshot: (dict | None) = None
        if self.__snapshot:
            snapshot = dataclasses.asdict(self.__snapshot)
            del snapshot["headers"]
    ...
    def __ensure_http_session2(self) -> aiohttp.ClientSession:
        if not self.__http_session:
            kwargs: dict = {
                "headers": {"User-Agent": htclient.make_user_agent("KVMD")},
                "connector": aiohttp.UnixConnector(path="/run/kvmd/ustreamer2.sock"),
                "timeout": aiohttp.ClientTimeout(total=self.__timeout),
            }
            self.__http_session = aiohttp.ClientSession(**kwargs)
        return self.__http_session
    ...
    async def __start_streamer_proc(self) -> None:
        ...
        self.__streamer_proc = await aioproc.run_process(cmd)
        get_logger(0).info("Started streamer2 pid=%d: %s", self.__streamer_proc.pid, tools.cmdfmt(cmd))
        cmd[1] = '--device=/dev/video2'
        cmd[8] = '--unix=/run/kvmd/ustreamer2.sock'
        get_logger(0).info("USTREAMER CMD2: ", cmd)
