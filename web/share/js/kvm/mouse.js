...
export function Mouse5(__getGeometry, __recordWsEvent) {
	var self = this;
	...
	var __init__ = function() {
		...
		document.onpointerlockchange = __relativeCapturedHandler; // Only for relative
		document.onpointerlockerror = __relativeCapturedHandler;
		$("stream-box5").onmouseenter = () => __streamHoveredHandler(true);
		$("stream-box5").onmouseleave = () => __streamHoveredHandler(false);
		$("stream-box5").onmousedown = (event) => __streamButtonHandler(event, true);
		$("stream-box5").onmouseup = (event) => __streamButtonHandler(event, false);
		$("stream-box5").oncontextmenu = (event) => event.preventDefault();
		$("stream-box5").onmousemove = __streamMoveHandler;
		$("stream-box5").onwheel = __streamScrollHandler;
		$("stream-box5").ontouchstart = (event) => __streamTouchStartHandler(event);
		$("stream-box5").ontouchmove = (event) => __streamTouchMoveHandler(event);
		$("stream-box5").ontouchend = (event) => __streamTouchEndHandler(event);
	...
	self.setSocket = function(ws) {
		__ws = ws;
		$("stream-box5").classList.toggle("stream-box-mouse-enabled", ws);
		if (!__absolute && __isRelativeCaptured()) {
			document.exitPointerLock();
		}
		__updateOnlineLeds();
	...
	var __isRelativeCaptured = function() {
		return (document.pointerLockElement === $("stream-box5"));
	};
	...
	var __streamButtonHandler = function(event, state) {
		...
		if (__absolute || __isRelativeCaptured()) {
			...
			$("stream-box5").requestPointerLock();
		}
	};
