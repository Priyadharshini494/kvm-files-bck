...
export function Msd() {
	...
	var __clickRemoveButton = function() {
		...
		wm2.confirm(`Are you sure you want to remove the image<br><b>${name}</b> from PiKVM?`).then(function(ok) {
			if (ok) {
				let http = tools.makeRequest("POST", `/api/msd/remove?image=${name}`, function() {
					if (http.readyState === 4) {
						if (http.status !== 200) {
							wm2.error("Can't remove image:<br>", http.responseText);
						}
					}
				});
			}
		...
		let http = tools.makeRequest("POST", `/api/msd/set_params?${name}=${encodeURIComponent(value)}`, function() {
			if (http.readyState === 4) {
				if (http.status !== 200) {
					...
					wm2.error("Can't configure MSD:<br>", http.responseText);
				}
	...
	var __httpStateChange = function() {
		if (__http.readyState === 4) {
			if (__http.status !== 200) {
				...
				wm2.error("Can't upload image to the Mass Storage Drive:<br>", __http.responseText);
		...
		let http = tools.makeRequest("POST", `/api/msd/set_connected?connected=${connected}`, function() {
			if (http.readyState === 4) {
				if (http.status !== 200) {
					...
					wm2.error("Switch error:<br>", http.responseText);
				}
			}
			__applyState();
		});
	...
	var __clickResetButton = function() {
		...
		wm2.confirm("Are you sure you want to reset Mass Storage Drive?").then(function(ok) {
			if (ok) {
				let http = tools.makeRequest("POST", "/api/msd/reset", function() {
					if (http.readyState === 4) {
						if (http.status !== 200) {
							wm2.error("MSD reset error:<br>", http.responseText);
						}
					}
					__applyState();
				});
	...
	var __selectNewFile = function() {
		...
		if (file) {
			...
			if (file.size > __state.storage.size) {
				...
				wm2.error("New image is too big for your Mass Storage Drive.<br>Maximum:", tools.formatSize(__state.storage.size));
				el_input.value = "";
			}
		}
		__applyState();
