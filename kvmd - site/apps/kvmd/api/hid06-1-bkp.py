...
class HidApi:
    ...
    @exposed_http("POST", "/battery/simulation")
    async def simulate_battery(self, request: Request) -> Response:
        ...
        result = {
            "status": "success",
            "message": "Battery simulation successful",
    ...
    async def __getData_handler(self, _: Request) -> Response:
        
            master, slave = pty.openpty()
            minicom_command = ["minicom", "-b", "115200", "--capturefile", "/dev/null", "-o", "-D", "/dev/ttyAMA0"]
            minicom_process = await asyncio.create_subprocess_exec(*minicom_command, stdin=slave, stdout=slave, stderr=slave, preexec_fn=os.setsid)
#            minicom_command = ["minicom", "-b", "115200", "-o", "-D", "/dev/ttyAMA0"]
#            minicom_process = subprocess.Popen(minicom_command, stdin=slave, stdout=slave, stderr=slave, preexec_fn=os.setsid)
            ...
            while True:
                    ...
                    if master in rlist:
                        line = os.read(master, 4096).decode('utf-8').strip()
                    #  print("Received:", line)
                    else:
