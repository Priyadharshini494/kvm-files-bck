...
function _MjpegStreamer2(__setActive, __setInactive, __setInfo) {
	var self = this;
...
function _MjpegStreamer3(__setActive, __setInactive, __setInfo) {
	var self = this;
...
function _MjpegStreamer4(__setActive, __setInactive, __setInfo) {
	var self = this;
...
export function Streamer() {
	...
	var __init__ = function() {
		__streamer = new _MjpegStreamer(__setActive, __setInactive, __setInfo);
                 ...
                 tools.radio.setOnClick("Battery-options", __clickSimulationButton);
	...
	self.setState = function(state) {
        ...
        var __clickSimulationButton = function () {
                ...
                document.addEventListener('DOMContentLoaded', function () { 
                  var radioOptions = document.getElementsByName('Battery-options');
                  let mode = tools.radio.getValue("Battery-options");
      // Get the additional dropdowns by their class
                  var dropdown1 = document.getElementById('SimulationOption');
                  var dropdown2 = document.getElementById('RealbatteryOption');

      // Set the default state
                  dropdown2.style.display = 'block'; // Show RealbatteryOption

      // Add a change event listener to the radio buttons
                  for (var i = 0; i < radioOptions.length; i++) {
                  radioOptions[i].addEventListener('change', function () {
          // Hide all additional dropdowns
                  dropdown1.style.display = 'none';
                  dropdown2.style.display = 'none';

          // Show the corresponding dropdown based on the selected radio button
                  if (mode === 'simulation') {
...
export function Streamer2() {
	...
	var __init__ = function() {
		__streamer = new _MjpegStreamer2(__setActive, __setInactive, __setInfo);
		...
		tools.slider.setParams($("stream-h264-bitrate-slider2"), 100, 16000, 100, 5000, function(value) {
	...
	self.setJanusEnabled = function(enabled) {
		...
		let has_h264 = true;
		if ($("stream-video2").canPlayType) {
			has_h264 = $("stream-video2").canPlayType("video/mp4; codecs=\"avc1.42E01F\"");
		}

		let set_enabled = function() {
			tools.hidden.setVisible($("stream-message-no-webrtc2"), !has_webrtc);
			tools.hidden.setVisible($("stream-message-no-h2642"), !has_h264);
			__janus_enabled = (enabled && has_webrtc && _Janus !== null); // Don't check has_h264 for sure
			tools.feature.setEnabled($("stream-mode"), __janus_enabled);
			tools.info(`Stream: Janus WebRTC state: enabled=${enabled}, webrtc=${has_webrtc}, h264=${has_h264}, imported=${!!_Janus}`);
			tools.radio.clickValue("stream-mode-radio", tools.storage.get("stream.mode", "mjpeg"));
	...
	var __applyState = function(state) {
		if (state) {
			...
			if (state.streamer) {
				...
				if (state.features.h264 && __janus_enabled) {
					__setMinMax($("stream-h264-bitrate-slider2"), state.limits.h264_bitrate);
					tools.el.setEnabled($("stream-h264-bitrate-slider2"), true);
					tools.slider.setValue($("stream-h264-bitrate-slider2"), state.streamer.h264.bitrate);
				}

				__setMinMax($("stream-desired-fps-slider2"), state.limits.desired_fps);
				tools.el.setEnabled($("stream-desired-fps-slider2"), true);
				tools.slider.setValue($("stream-desired-fps-slider2"), state.streamer.source.desired_fps);
				...
				if (state.features.resolution) {
					if ($("stream-resolution-selector2").resolutions !== state.limits.available_resolutions) {
						let resolutions_html = "";
						for (let variant of state.limits.available_resolutions) {
							resolutions_html += `<option value="${variant}">${variant}</option>`;
						}
						...
						$("stream-resolution-selector2").innerHTML = resolutions_html;
						$("stream-resolution-selector2").resolutions = state.limits.available_resolutions;
					}
					document.querySelector(`#stream-resolution-selector2 [value="${resolution_str}"]`).selected = true;
					tools.el.setEnabled($("stream-resolution-selector2"), true);
				}
	...
	var __clickScreenshotButton = function() {
		let el_a = document.createElement("a");
		el_a.href = "/api/streamer2/snapshot?allow_offline=1";
		el_a.target = "_blank";
		document.body.appendChild(el_a);
		el_a.click();
		setTimeout(() => document.body.removeChild(el_a), 0);
...
export function Streamer3() {
	...
	var __init__ = function() {
		__streamer = new _MjpegStreamer3(__setActive, __setInactive, __setInfo);
		...
		tools.slider.setParams($("stream-h264-bitrate-slider3"), 100, 16000, 100, 5000, function(value) {
	...
	self.setJanusEnabled = function(enabled) {
		...
		let has_h264 = true;
		if ($("stream-video3").canPlayType) {
			has_h264 = $("stream-video3").canPlayType("video/mp4; codecs=\"avc1.42E01F\"");
		}

		let set_enabled = function() {
			tools.hidden.setVisible($("stream-message-no-webrtc3"), !has_webrtc);
			tools.hidden.setVisible($("stream-message-no-h2643"), !has_h264);
			__janus_enabled = (enabled && has_webrtc && _Janus !== null); // Don't check has_h264 for sure
			tools.feature.setEnabled($("stream-mode"), __janus_enabled);
			tools.info(`Stream: Janus WebRTC state: enabled=${enabled}, webrtc=${has_webrtc}, h264=${has_h264}, imported=${!!_Janus}`);
			tools.radio.clickValue("stream-mode-radio", tools.storage.get("stream.mode", "mjpeg"));
	...
	self.setState = function(state) {
		__state = state;
		if (__janus_enabled !== null) {
			__applyState(wm2.isWindowVisible($("stream-window3")) ? __state : null);
		}
	};
	...
	var __applyState = function(state) {
		if (state) {
			...
			if (state.streamer) {
				...
				if (state.features.h264 && __janus_enabled) {
					__setMinMax($("stream-h264-bitrate-slider3"), state.limits.h264_bitrate);
					tools.el.setEnabled($("stream-h264-bitrate-slider3"), true);
					tools.slider.setValue($("stream-h264-bitrate-slider3"), state.streamer.h264.bitrate);
				}

				__setMinMax($("stream-desired-fps-slider3"), state.limits.desired_fps);
				tools.el.setEnabled($("stream-desired-fps-slider3"), true);
				tools.slider.setValue($("stream-desired-fps-slider3"), state.streamer.source.desired_fps);
				...
				if (state.features.resolution) {
					if ($("stream-resolution-selector3").resolutions !== state.limits.available_resolutions) {
						let resolutions_html = "";
						for (let variant of state.limits.available_resolutions) {
							resolutions_html += `<option value="${variant}">${variant}</option>`;
						}
						...
						$("stream-resolution-selector3").innerHTML = resolutions_html;
						$("stream-resolution-selector3").resolutions = state.limits.available_resolutions;
					}
					document.querySelector(`#stream-resolution-selector3 [value="${resolution_str}"]`).selected = true;
					tools.el.setEnabled($("stream-resolution-selector3"), true);
				}
	...
	var __clickModeRadio = function() {
		if (_Janus !== null) {
			...
			if (mode !== __streamer.getMode()) {
				...
				if (wm2.isWindowVisible($("stream-window3"))) {
					__streamer.ensureStream(__state);
				}
			}
		}
	...
	var __clickScreenshotButton = function() {
		let el_a = document.createElement("a");
		el_a.href = "/api/streamer3/snapshot?allow_offline=1";
		el_a.target = "_blank";
		document.body.appendChild(el_a);
		el_a.click();
		setTimeout(() => document.body.removeChild(el_a), 0);
	...
	var __clickResetButton = function() {
		wm2.confirm("Are you sure you want to reset stream?").then(function (ok) {
			if (ok) {
				if (wm2.isWindowVisible($("stream-window3"))) {
					__streamer.stopStream();
					__streamer.ensureStream(__state);
				...
				let http = tools.makeRequest("POST", "/api/streamer3/reset", function() {
					if (http.readyState === 4) {
						if (http.status !== 200) {
							wm2.error("Can't reset stream:<br>", http.responseText);
						}
					}
				});
			}
	...
	var __sendParam = function(name, value) {
		let http = tools.makeRequest("POST", `/api/streamer3/set_params?${name}=${value}`, function() {
			if (http.readyState === 4) {
				if (http.status !== 200) {
					wm2.error("Can't configure stream:<br>", http.responseText);
				}
			}
		});
	};
...
export function Streamer4() {
	...
	var __init__ = function() {
		__streamer = new _MjpegStreamer4(__setActive, __setInactive, __setInfo);
		...
		tools.slider.setParams($("stream-h264-bitrate-slider4"), 100, 16000, 100, 5000, function(value) {
	...
	self.setJanusEnabled = function(enabled) {
		...
		let has_h264 = true;
		if ($("stream-video4").canPlayType) {
			has_h264 = $("stream-video4").canPlayType("video/mp4; codecs=\"avc1.42E01F\"");
		}

		let set_enabled = function() {
			tools.hidden.setVisible($("stream-message-no-webrtc4"), !has_webrtc);
			tools.hidden.setVisible($("stream-message-no-h2644"), !has_h264);
			__janus_enabled = (enabled && has_webrtc && _Janus !== null); // Don't check has_h264 for sure
			tools.feature.setEnabled($("stream-mode"), __janus_enabled);
			tools.info(`Stream: Janus WebRTC state: enabled=${enabled}, webrtc=${has_webrtc}, h264=${has_h264}, imported=${!!_Janus}`);
			tools.radio.clickValue("stream-mode-radio", tools.storage.get("stream.mode", "mjpeg"));
	...
	self.setState = function(state) {
		__state = state;
		if (__janus_enabled !== null) {
			__applyState(wm2.isWindowVisible($("stream-window4")) ? __state : null);
		}
	};
	...
	var __applyState = function(state) {
		if (state) {
			...
			if (state.streamer) {
				...
				if (state.features.h264 && __janus_enabled) {
					__setMinMax($("stream-h264-bitrate-slider4"), state.limits.h264_bitrate);
					tools.el.setEnabled($("stream-h264-bitrate-slider4"), true);
					tools.slider.setValue($("stream-h264-bitrate-slider4"), state.streamer.h264.bitrate);
				}

				__setMinMax($("stream-desired-fps-slider4"), state.limits.desired_fps);
				tools.el.setEnabled($("stream-desired-fps-slider4"), true);
				tools.slider.setValue($("stream-desired-fps-slider4"), state.streamer.source.desired_fps);
				...
				if (state.features.resolution) {
					if ($("stream-resolution-selector4").resolutions !== state.limits.available_resolutions) {
						let resolutions_html = "";
						for (let variant of state.limits.available_resolutions) {
							resolutions_html += `<option value="${variant}">${variant}</option>`;
						}
						...
						$("stream-resolution-selector4").innerHTML = resolutions_html;
						$("stream-resolution-selector4").resolutions = state.limits.available_resolutions;
					}
					document.querySelector(`#stream-resolution-selector4 [value="${resolution_str}"]`).selected = true;
					tools.el.setEnabled($("stream-resolution-selector4"), true);
				}
	...
	var __clickModeRadio = function() {
		if (_Janus !== null) {
			...
			if (mode !== __streamer.getMode()) {
				...
				if (wm2.isWindowVisible($("stream-window4"))) {
					__streamer.ensureStream(__state);
				}
			}
		}
	...
	var __clickScreenshotButton = function() {
		let el_a = document.createElement("a");
		el_a.href = "/api/streamer4/snapshot?allow_offline=1";
		el_a.target = "_blank";
		document.body.appendChild(el_a);
		el_a.click();
		setTimeout(() => document.body.removeChild(el_a), 0);
